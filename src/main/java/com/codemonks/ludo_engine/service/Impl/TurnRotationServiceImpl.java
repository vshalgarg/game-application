...
public class TurnRotationServiceImpl implements TurnRotationService {
    ...
    {
        log.info("[TURN_ROTATION_STARTED] Player:{} ExtraTurn:{}",
                currentPlayerId, extraTurn);
        ...
        if (currentPlayer == null) {
            log.error("[CURRENT_PLAYER_NOT_FOUND] Player:{}", currentPlayerId);
        ...
        for (int step = 1; step <= players.size(); step++) {
            int candidateIndex = (currentPlayerIndex + step) % players.size();
            PlayerDTO candidate = players.get(candidateIndex);

            if (!Boolean.TRUE.equals(candidate.getHasFinished())) {
                nextPlayerId = candidate.getPlayerId();
                break;
            }
        }
        ...
        if (nextPlayerId == null) {
            log.warn(
                    "[NO_ACTIVE_PLAYER_FOUND] All players finished. CurrentPlayer:{}",
                    currentPlayerId
            );
            nextPlayerId = players.get((currentPlayerIndex + 1) % players.size()).getPlayerId();
        }

        gameState.setCurrentTurnPlayerId(nextPlayerId);

        log.info("[TURN_ROTATED] FromPlayer:{} ToPlayer:{}", currentPlayerId,
                nextPlayerId);
        return gameState;
}}